  ...
  public function index(Request $request)
{
    ...
    public static function getAccessToken() {
        $url = 'https://teebooking.aepta.in/AEPTAServiceAccountKey.json';
...
//                   ->where('device_id', '!=', '')
//                   ->pluck('id', 'device_id') // [token => user_id]
//                   ->toArray();

// $tokenChunks = array_chunk($allTokens, 500, true); // ✅ preserve token => user_id
...
// }
public function showSentUsers(Request $request, $id)
{
    $notification = Notifications::findOrFail($id);

    $query = NotificationUser::with('user')
        ->where('notification_id', $id)
        ->where('success', true);
...
{
    $reminders = Notifications::all(); // Fetch all reminders
    return view('admin.notifications.config.reminders', compact('reminders'));
}
