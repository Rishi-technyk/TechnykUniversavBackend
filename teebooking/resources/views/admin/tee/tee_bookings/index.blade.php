                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><a href="{{ route('tee.tee_sheets.show', ['id' => $teeBooking->id]) }}">
                                    {{ \Carbon\Carbon::parse($teeBooking->booking_date)->format('d-m-Y') }}
                            ...
                            <td>
                                <label class="switch switch-status">
                                    <input type="checkbox" class="status" id="{{ $teeBooking->id }}"
                                        {{$teeBooking->is_active == 1?'checked':''}}>
                                    <span class="slider round"></span>
                                </label>
                            </td>
                            <td>
                                <a href="{{ route('tee_bookings.edit', $teeBooking->id) }}"
                                    class="btn btn-primary">Edit</a>
                                <form action="{{ route('tee_bookings.destroy', $teeBooking->id) }}" method="POST"
                                    style="display: inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-danger"
                                        onclick="return confirm('Are you sure you want to delete this tee sheet?')">Delete</button>
                                </form>
                            </td>
                        </tr>
                        @endforeach
...
$(document).ready(function() {
    ...
    $('.datatable-input').attr('placeholder', 'Search Booking Date,Time');
    $('.datatable-input').css('width', '250px');
});
