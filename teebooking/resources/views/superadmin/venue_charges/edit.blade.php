...
<div class="card mb-1 h-100">
    ...
    <div class="card-body">
        ...
        <form action="{{ route('venue.charge.update') }}" method="Post">
            @csrf
            ...
            <div class="row">
                ...
                <div class="col-lg-4">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="occupant_id" required>
                            ...
                            <option value="{{ $occ->id }}" {{ $data->occupant_id==$occ->id?'selected':'' }}>{{ $occ->name }}</option>
                            @endforeach
                        </select>
                ...
                <?php
                    $minPaxArray = is_string($data->min_pax) ? json_decode($data->min_pax, true) : $data->min_pax;
                    $maxPaxArray = is_string($data->max_pax) ? json_decode($data->max_pax, true) : $data->max_pax;
                    $rateArray   = is_string($data->rate) ? json_decode($data->rate, true) : $data->rate;
                ?>
                ...
                @if($rateArray)
                    ...
                    @foreach($rateArray as $key => $rate)
                        ...
                        <div class="col-lg-4 mt-4 {{ $key }}">
                            
                            <div class="form-group">
                                
                                <label>Min Pax</label>
                                <input type="number" name="min_pax[]" class="form-control" value="{{ $minPaxArray[$key] }}" placeholder="Enter Min Pax" required>
                        ...
                        <div class="col-lg-4 mt-4 {{ $key }}">
                            ...
                            <div class="form-group">
                                ...
                                <label>Max Pax</label>
                                <input type="number" name="max_pax[]" class="form-control" value="{{ $maxPaxArray[$key] }}" placeholder="Enter Max Pax" required>

                            </div>

                        </div>

                        <div class="col-lg-3 mt-4 {{ $key }}">
                            
                            <div class="form-group">
                                
                                <label>Rate</label>
                                <input type="number" name="rate[]" class="form-control" value="{{ $rate }}" placeholder="Enter Rate" required>

                            </div>

                        </div>

                        <div class="col-lg-1 mt-4 form-group {{ $key }}"><br>
                            <button class="btn btn-danger btn-sm mt-2" onclick="deleteRow({{ $key }})" type="button"><i class="bi bi-trash"></i> Remove</button>
                        </div>
