...
class FCMService
{
    ...
    {
        $this->jsonPath = storage_path('app/firebase/holidayclub-service-account.json');

        if (!file_exists($this->jsonPath)) {
            Log::error('Firebase service account JSON not found', ['path' => $this->jsonPath]);
        }
    }
    ...
    public function sendNotification(string $deviceToken, string $title, string $body, array $data = []): bool
    {
        ...
        $credentials = json_decode(file_get_contents($this->jsonPath), true);
        ...
        try {
            ...
            if ($response->successful()) {
                Log::info('FCM sent successfully', ['token' => $deviceToken]);
                return true;
            }
    ...
    /**
     * Generate OAuth Access Token manually from Service Account JSON
     */
